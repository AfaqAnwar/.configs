return {
  ...
  opts = function(_, opts)
    ...
    local function custom_branch()
      ...
      return (branch == nil or branch == "") and "" or (" " .. branch)
    end

    for _, mode in pairs(auto) do
      for _, sec in pairs(mode) do
        if type(sec) == "table" and sec.bg then
          sec.bg = "NONE"
        end
      end
    end
      ...
      lualine_a = {
        {
          ...
          color = function()
            ...
            if mode == "\22" then
              return { fg = "none", bg = "NONE", gui = "bold" }
            elseif mode == "V" then
        ...
        separator(),
        {
          function()
            ...
            for i, buf in ipairs(bufnr_list) do
              if buf.bufnr == current_bufnr then current_index = i; break end
            end
